...
enum operator { nothing, plus, minus, product, divide } op;
gchar str[100];
gint cnt;
...
void numberButtonClicked(GtkButton *button, gpointer data) {
    ...
    if(isFirst) num1 = strtod(str, NULL);
    else {
...
void dotButtonClicked(GtkButton *button, gpointer data) {
    if(cnt == 0) gtk_entry_set_text(GTK_ENTRY(entry), "0.");
    else {
        if(haveDot == FALSE) {
            haveDot = TRUE;
            gint i = g_strlcat(str, ".", 100);
            if(isFirst) num1 = strtod(str, NULL);
            else num2 = strtod(str, NULL);
